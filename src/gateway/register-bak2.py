						...
						if buf:
							...
							if result['flags']==0:	
								if result['Mac_address'] not in Register_Del.mac_dev_map:
									...
									for i in range(res_num):
										if result["Mac_address"]+"\t"+str(result['Res'][i]['Res_port']) not in Register_Del.mac_resID_resPlat_map:
											res_id=WrtGateway.add_res(dev_id)
								...
								for i in Register_Del.mac_resID_resPlat_map:
									keys=i.split("\t")
									if len(keys)==2:
										if keys[0]==result["Mac_address"]:
											del Register_Del.mac_resID_resPlat_map[i]
